...
function parse_commandline()
    ...
    @add_arg_table s begin
        "--selection"
            help = "list of gene IDs to select directly inside your fasta transcriptome file."
            action = :append_arg
            nargs = '*'
        "--appris"
            help = """indicate: 'homo_sapiens', 'mus_musculus', 'rattus_norvegicus',
                ...
                issue; 2017 Oct 23]. A gene has multiple possible sequences depending
                on its variants. This option selects the principal transcript, based
                on APPRIS database. If this option is used and there is no data
                available, no connection or no principal transcript (non-coding genes),
                the longer transcript is selected."""
            arg_type = String
        "--unannotated", "-u"
            action = :store_true
            help = """use this option if the provided initial fasta file corresponds
                to an annotation external from Ensembl. Otherwise, use ensembl fasta
                files!"""
        "--stringent", "-s"
            action = :store_true
            help = """if you think a gene-specific tag must be unique but present in ALL
            KNOWN TRANSCRIPTS for this gene. If false, the kmer is considered as gene-
            specific if present in at least one variant of the corresponding gene."""
        "--verbose", "-v"
            action = :store_true
            help = "if you want this script to talk too much"
        "--genome", "-g"
            help = "genome fasta (.fa) or jellyfish index (.jf) to use for kmer request."
            #type = String
            required = true
        "--transcriptome", "-t"
            help = """transcriptome fasta (.fa) (FASTA ONLY) to use for kmer request
                and transcriptional variants informations."""
            #type = String
            required = true
        "--level", "-l"
        ...
        "--output", "-o"
            ...
            default = "output"
        "--length"
            help = "length required for the kmer generation (default 31)."
            default = 31
            arg_type = Int
        "--procs", "-p"
...
function set_variables()
    ...
    if verbose_option println("
        ...
        \r Genes selection option:    ", length(select_option)>0 ? (select_option, ", ") : "nothing", "
...
## Check the conformity of the variables
function checkup_variables()
    ...
    ## test fasta file
    if ! isfile(fastafile)
        println("\nFileError: sequences file not found (file: $fastafile)\n")
        exit(2)
    ...
    if select_option != nothing
        for gene in select_option
            # ENSEMBL annotation are ENSTXXXX.X or ENSGXXXXX.X. To avoid bad recognition
            # for selection option, annotation like ENSTXXXXX.X is forbidden (remove .X)
...
function run_jellyfish(genome, transcriptome_fa)
    ...
    if occursin(r".*\.(fa|fasta)$", genome)
        ...
        println("Jellyfish genome index already provided, nothing to do")
...
function build_sequences()
    ...
    ## split each sequence of the fasta input file into individual fastas
    ...
    if ! unannotated_option
        ...
        FastaReader(fastafile) do fr
            for (desc, seq) in fr
                ...
                ## engage this loop only if the gene has not been already processed. Otherwise, the gene will be processed n times, n being the nb of transcripts for the same gene in the reference fasta.
                if isempty(select_option) || (select_option != nothing && (gene_name in select_option || ensembl_transcript_name in select_option || ensembl_gene_name in select_option)) && (gene_name in genes_already_processed) == false
                    ## take all sequences corresponding to asked gene names (select option)
                    ...
                    if APPRIS_option == nothing || (APPRIS_option != nothing && ensembl_transcript_name == APPRIS_transcript) || (APPRIS_option != nothing && APPRIS_transcript == "NODATA" && "$gene_name:$ensembl_transcript_name" == find_longest_variant(gene_name,transcriptome_dict))
                        ...
                        if length("$seq") >= kmer_length
                            ...
                            gene_name = replace(gene_name, "/" => "@SLASH@") # some genes names can contain slash characters, break the processus
                            ...
                            println("$gene_name:$ensembl_transcript_name : sequence length < $kmer_length => ignored")
                        end
                    end
                end
            end
            ...
            for i in unique(genes_analysed)
                ## it can happen that APPRIS contains obsolete IDs/error, in this case the longer variant is selected for the concerned gene (passed in loop but has not provided a "sequence" fasta)
    ...
    if verbose_option println("Sequences splitting into individual files done \n \r ------------ \n") end
end


## Parse arguments + set variables and check them
...
checkup_variables()
transcriptome_dict = load_transcriptome(transcriptome) #transcriptome dictionary
...
## Extraction of specific kmers for one sequence of the fasta files
@everywhere function f(splitted_fasta_files)
    ...
    if verbose_option remotecall(replace_line, 1 , X, "Jellyfish query on $transcriptome.jf done") end
    ...
    if level == "transcript" && unannotated_option == false && contig_string != ""
        push!(fasta_contig_array,">$gene_name-$transcript_name.contig$j")
        push!(fasta_contig_array,"$contig_string")
